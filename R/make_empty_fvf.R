...
#' 
#' @return databel_filtered_R object; also file is created in file system
#' 

make_empty_fvf <- function(name, nvariables, nobservations,type = "DOUBLE") 
{
	if (!is(name,"character") || length(name) > 1) stop("file name argument is either non-character or non-scalar");
	...
	return(databel_filtered_R(as.character(name)))
